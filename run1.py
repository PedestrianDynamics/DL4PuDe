...
def label(mim,name):
    ...
    for patch in range (0,len(x1)):
         ...
         if classify(mimPatch)==1:
            ...
            cate=0
         end4 = time.time()
         print(f"Runtime of one prediction is {end4 - start4}")  
         rec.append(cate)
    return (rec)
##############################################################################################
...
def vizualize_flow(img, flo, counter,roi):
    ...
    ###########################################
    end3 = time.time()
    print(f"Runtime of the WV is {end3 - start3}")  
